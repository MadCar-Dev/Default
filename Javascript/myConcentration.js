...
API_Meta.myConcentration = { offset: Number.MAX_SAFE_INTEGER, lineCount: -1 };
{
  try { throw new Error(''); } catch (e) { API_Meta.myConcentration.offset = (parseInt(e.stack.split(/\n/)[1].replace(/^.*:(\d+):.*$/, '$1'), 10) - (4)); }
}
/*
 * Version 0.1.14
 * Made By Robin Kuiper /tweaked by Will M.
 * TWeaks include:  Add/Remove Turnorder item for concentration
 *                  Add/Remove tooltip
 *                  Added a reference to my custom Status Marker 
*/
...
var Concentration = Concentration || (function() {
            ...
            playerIsGM(playerid)){
                if(!token.get('status_'+marker)){
                    ...
                    if(spell){
                        ...
                        token.set('tooltip', `is concentrating on ${spell}.`)
                        to_AddCustom(`*** ${token.get('name')} concentrating on ${spell}`, 1, '+1')

                    }else{
                        message = '<b>'+token.get('name')+'</b> is now concentrating.';
                        token.set('tooltip', `is now concentrating.`)
                        to_AddCustom(`*** ${token.get('name')} is concentrating`, 1, '+1')
    ...
    handleStatusMarkerChange = (obj /*, prev */) => {
        ...
        if(!obj.get('status_'+marker)){
            ...
            to_RemoveCustom(`*** ${obj.get('name')} concentrating`)
            obj.set('tooltip', ' ')
        }
    },
    ...
    function to_RemoveCustom(itemName) {
      const turnOrder = JSON.parse(Campaign().get('turnorder'));
      let ndx = -1;
      ...
      // Find the turnOrder Item Index to remove
      for (let i = 0; i<= turnOrder.length -1; i++) {
        if (turnOrder.custom.startsWith(itemName)) {
...
on('ready',function() {
    'use strict';
    log('>>>-----> myConcentration is ready! --offset '+ API_Meta.myConcentration.offset);
    Concentration.CheckInstall();
    Concentration.RegisterEventHandlers();
});
{try{throw new Error('');}catch(e){API_Meta.myConcentration.lineCount=(parseInt(e.stack.split(/\n/)[1].replace(/^.*:(\d+):.*$/,'$1'),10)-API_Meta.myConcentration.offset);}}