...
return ((url.includes('marketplace')) ? 'border: 2px solid #000;; ' : 'border: 2px solid #aaa;');
    ...
    function makeImageLink(url) {
        ...
        url = url.replace('med.', 'thumb.') || url;
        url = url.replace('max.', 'thumb.') || url;
        url = url.replace('original.', 'thumb.') || url;
...
/*graphics = graphics.filter(function(obj){
  return obj.get('imgsrc').includes(imageFilter);
});*/
            ...
            case 'url':
                ...
                pageList = `<div style = 'margin: 5px' ${makeBigImageLink(imgurl)}`+ ((imgurl.includes("marketplace")) ? " Marketplace image" : " User Library image") +`</div>${openHeader}Count Per Page:${closeHeader} <BR>`;
