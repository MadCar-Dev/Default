...
// Future Enhancements
//  1. Replace Group-Init logic
//  2. Place the menus in a pleasing shadowbox 
//  3. change the base command of !dmr to !tor
//  4. replace some of my other gmnotes/notes update fields with the new asyn method
//  5. Create button or link to popup the Handout Char Sheet/Log/Report


var API_Meta = API_Meta || {};
API_Meta.DMReporter = { offset: Number.MAX_SAFE_INTEGER, lineCount: -1 };
{
  try { throw new Error(''); } catch (e) { API_Meta.DMReporter.offset = (parseInt(e.stack.split(/\n/)[1].replace(/^.*:(\d+):.*$/, '$1'), 10) - (4)); }
}
...
on('ready', () => {
  const version = '0.5.0';
  log('DM Turnorder Reporter' + version + ' is ready! --offset '+ API_Meta.DMReporter.offset);
...
on('chat:message', async (msg_orig) => {
  ...
  if (!/^!dmr/.test(msg.content)) {
    return;
  }
  log('HO Event: chat:message');
  let ret = handleMsg(msg.content);
});
...
function handleMsg(msg_content){
  ...
  commands.forEach(c => {
    ...
    switch (c) {
      ...
      case 'TO-AddCountdown': // Direction, Starting Pos, Name
        ...
        break;
      case 'TO-Remove':
        to_Remove(commands[1]); // Assumes that the next the command is "!dmr --to-Remove tid"
        updateTurnOrderStartTime();
        refreshReports();
        break;
      case 'TO-RemoveCustom':
        to_RemoveCustom(commands[1]); // Assumes that the next the command is "!dmr --to-RemoveCustom ndx"
        updateTurnOrderStartTime();
        refreshReports();
        break;
      ...
      case 'TOReport':
        ...
        if(Exp_ndx>0){
          state.DMReporter.DetailExpand = 1;
        }
        Exp_ndx = msg_content.indexOf('collapse');
        if(Exp_ndx>0){
          state.DMReporter.DetailExpand = 0;
        }
        refreshReports();
        break;
    default:
...
function updateTurnOrderStartTime(){
  state.DMReporter.LastDT1 = getSystemTimeInSecs();
  state.DMReporter.LastUTCDate = GetSystemUTCDate();
}
...
function DidTOAdvance(){
  ...
  let prev_to = state.DMReporter.PrevTO;
  let curr_to = Campaign().get('turnorder');
  state.DMReporter.PrevTO = curr_to;
...
// Purpose:  Write a csv record to a handout called 
  ...
  //Get Last System Time state.DMReporter.LastDT1
  let prevDate = state.DMReporter.LastUTCDate;
...
function changeAttributeValue(characterId, attributeName, newValue) {
...
function ResetStats(){
  ...
  state.DMReporter.DM_Secs = 0;
  state.DMReporter.DM_Count = 0;
  state.DMReporter.DM_Avg = 0;
}
...
function refreshReports() {
  ...
  const openBox = "<div style='color: #000; border: 1px solid #000; background-color: #FFEBD6; box-shadow: 0 0 3px #000; display: block; text-align: left; font-size: 14px; padding: 2px; margin-bottom: 2px; font-family: sans-serif; white-space: pre-wrap;'>";
  const closeBox = '</div>';
  ...
  let cmdRefresh = '&#13;!dmr --TOReport';
  let btnRefresh = makeButton('Refresh!', '!dmr --TOReport');
  let btnExp = '   ' + makeButton('[+] Expand Current Character Details', '!dmr --TOReport expand');
  let btnCps = '   ' + makeButton('[-] Collapse Current Character Details', '!dmr --TOReport collapse');

  let btnPrev = makeButton('Prev',`!dmr --TO-Prev`);
  let btnNext = makeButton('Next',`!dmr --TO-Next`);
  let btnSort = makeButton('Sort',`!dmr --TO-Sort`);
  let btnSortWrapped = makeButton('Sort-Wrap',`!dmr --TO-SortWrapped`);
  let btnClear = makeButton('Clear',`!dmr --TO-Clear`);
  ...
  let btnRound = makeButton('Round', `!dmr --TO-AddRound`);
  let btnCounter = makeButton('Counter', `!dmr --TO-AddCountdown ?{Counter direction|Count Down,-1|Count Up,+1} ?{Counter starting position|10} "?{Counter Name|}"`);
  let btnCustom = makeButton('Custom', `!dmr --TO-AddCustom ?{Custom turnorder position|10} "?{Custom turnorder Name|}"`);
  let btnResetStats = makeButton('Reset-Stats', `!dmr --ResetStats`);
  ...
  if (toObj.length >0) { 
    ...
    let dt1 = state.DMReporter.LastDT1;
        ...
        if ((dt_diff >= 0) && toObj[toObj.length-1].id != -1) {
          ...
          switch (Ttype) {
            ...
            case 'CHAR':
              ...
              changeAttributeValue(toPrevChar.get('_id'), 'to_secs', TO_Secs)
              changeAttributeValue(toPrevChar.get('_id'), 'to_count', TO_Count)
              changeAttributeValue(toPrevChar.get('_id'), 'to_avg', TO_Avg)
            ...
            case 'NPC':  
              ...
              TO_Count = state.DMReporter.DM_Count;
              TO_Secs = state.DMReporter.DM_Secs;
              TO_Avg = state.DMReporter.DM_Avg;
              ...
              TO_Avg = Math.floor(TO_Secs / TO_Count);
              state.DMReporter.DM_Secs = TO_Secs;
              state.DMReporter.DM_Count = TO_Count;
              state.DMReporter.DM_Avg = TO_Avg;
              break;
          }
        }
      }
    ...
    ******************************************************************/
    ...
    switch (Ttype) {
      case 'OTHER':
        ...
        charheader = `${openHeader} ${makeButton('🎯', `!dmr --PingToken-GM ${toToken.get('_id')}`, 20)} ${tknImg} ${toToken.get('name')} - Graphic Token (Not associated with Character/NPC) Turnorder: (${toObj[0].pr})  ${closeHeader}<br>`;
        chartbl = '';
      ...
      case 'CHAR':
        ...
        chartbl += '<tr><td' + tdpc + '><b>Str:</b>' + getAttrByName(toChar.get('_id'),'strength','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'strength_mod','current')) +') sv:' + AddSign(getAttrByName(toChar.get('_id'),'strength_save_bonus','current')) + '</td></tr>';
        chartbl += '<tr><td' + tdpc + '><b>Dex:</b>' + getAttrByName(toChar.get('_id'),'dexterity','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'dexterity_mod','current')) +') sv:' + AddSign(getAttrByName(toChar.get('_id'),'dexterity_save_bonus','current')) + '</td></tr>';
        chartbl += '<tr><td' + tdpc + '><b>Con:</b>' + getAttrByName(toChar.get('_id'),'constitution','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'constitution_mod','current')) +') sv:' + AddSign(getAttrByName(toChar.get('_id'),'constitution_save_bonus','current')) + '</td></tr>';
        chartbl += '<tr><td' + tdpc + '><b>Wis:</b>' + getAttrByName(toChar.get('_id'),'wisdom','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'wisdom_mod','current')) +') sv:' + AddSign(getAttrByName(toChar.get('_id'),'wisdom_save_bonus','current')) + '</td></tr>';
        chartbl += '<tr><td' + tdpc + '><b>Int:</b>' + getAttrByName(toChar.get('_id'),'intelligence','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'intelligence_mod','current')) +') sv:' + AddSign(getAttrByName(toChar.get('_id'),'intelligence_save_bonus','current')) + '</td></tr>';
        chartbl += '<tr><td' + tdpc + '><b>Cha:</b>' + getAttrByName(toChar.get('_id'),'charisma','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'charisma_mod','current')) +') sv:' + AddSign(getAttrByName(toChar.get('_id'),'charisma_save_bonus','current')) + '</td></tr>';
        ...
        skills.forEach(mySkill => {
            ...
            chartbl += '<tr><td' + tdpc + '><b>' + skill_name + ':</b> ' + AddSign(skill_value) + '</td></tr>';
      ...
      case 'NPC':
        ...
        charheader = `${openHeader} ${charbtn} ${makeButton('🎯', `!dmr --PingToken-GM ${toToken.get('_id')}`, 20)} ${tknImg} ${toToken.get('name')}      [Avg Turn: ${state.DMReporter.DM_Avg} / Secs: ${state.DMReporter.DM_Secs} / Cnt: ${state.DMReporter.DM_Count}]  ${closeHeader} <b><i>${getAttrByName(toChar.get('_id'),'npc_type','current')}</b></i> `;
        ...
        chartbl += '<tr><td' + tdnpc + '><b>Str:</b>' + getAttrByName(toChar.get('_id'),'strength','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'strength_mod','current')) +')</td></tr>';
        chartbl += '<tr><td' + tdnpc + '><b>Dex:</b>' + getAttrByName(toChar.get('_id'),'dexterity','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'dexterity_mod','current')) +')</td></tr>';
        chartbl += '<tr><td' + tdnpc + '><b>Con:</b>' + getAttrByName(toChar.get('_id'),'constitution','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'constitution_mod','current')) +')</td></tr>';
        chartbl += '<tr><td' + tdnpc + '><b>Wis:</b>' + getAttrByName(toChar.get('_id'),'wisdom','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'wisdom_mod','current')) +')</td></tr>';
        chartbl += '<tr><td' + tdnpc + '><b>Int:</b>' + getAttrByName(toChar.get('_id'),'intelligence','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'intelligence_mod','current')) +')</td></tr>';
        chartbl += '<tr><td' + tdnpc + '><b>Cha:</b>' + getAttrByName(toChar.get('_id'),'charisma','current') + '(' + AddSign(getAttrByName(toChar.get('_id'),'charisma_mod','current')) +')</td></tr>';
        chartbl += '</table></td>'
        ...
        skills.forEach(mySkill => {
          ...
          let skill_value = skill.current;
          //log('debug5 ' + skill.current);
          if (skill_value != 0) {
            let skill_name = mySkill;
            chartbl += skill_name + ' ' + skill_value + ', ';
            // log('debug6 ' + skill_name + ' ' + skill_value);
          }
        });
        chartbl += '</td></tr>';
  ...
  if (state.DMReporter.DetailExpand == 1) {
  ...
  ******************************************************************/
    ...
    switch (Ttype) {
      case 'OTHER':
        ...
        if(toTkn.get('layer') == 'objects'){
          toList += '<td ' + tdcustom + '>' + makeButton(tknImg, `!dmr --PingToken-GM ${toTkn.get('_id')}`, 40) + '<b>' + toTkn.get('name') + '</b></td>';
        } else {
          toList += '<td ' + tdcustom + '>' + makeButton(tknImg, `!dmr --PingToken-GM ${toTkn.get('_id')}`, 40) + '<i>' + toTkn.get('name') + '</i></td>';
        }

        // Col 3 (Functions: Remove Item, Toggle Token between GM/Obj Layer)
        toList +=  '<td ' + tdcustom + '>' + makeButton('❌',`!dmr --TO-Remove ${toObj[i].id}`,20);
        if(toTkn.get('layer') == 'objects'){
          toList += makeButton('🙈', '!token-mod --set layer|gmlayer --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh , 20); 
        } else {
          toList += makeButton('🙉', '!token-mod --set layer|objects --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh , 20);
        }
        toList+='</td>'
        ...
        btnAdjHP = `!token-mod --set bar1_value|?{Adjust HP?|} --ignore-selected --ids ${toTkn.get('_id')}`
        btnAdjHP += ` &#13;&#37;{Mule|AdjHealthMarkers} ${cmdRefresh}`
        btnAdjHP = makeButton('🩹',btnAdjHP,20);
        ...
        if(toTkn.get('show_tooltip')){"?{Custom turnorder Name|}"
          toList += '<td ' + tdbase + '>' 
          toList += makeButton('🖊', '!token-mod --set tooltip|"?{Edit Tooltip|' + tt + '}" --ignore-selected --ids ' + toTkn.get('_id')) 
          toList += makeButton('❌', '!token-mod --set tooltip|?{Edit Tooltip| --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh) 
          toList += makeButton('🙈', '!token-mod --off show_tooltip --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh) 
          toList += '<b>' + tt + '</b></td>'; 
        } else {
          ...
          toList += makeButton('🖊', '!token-mod --set tooltip|"?{Edit Tooltip|' + tt + '}" --ignore-selected --ids ' + toTkn.get('_id')) 
          toList += makeButton('❌', '!token-mod --set tooltip|?{Edit Tooltip| --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh) 
          toList += makeButton('🙉', '!token-mod --on show_tooltip --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh) 
          toList += '<b>' + tt + '</b></td>'; 
      ...
      case 'CUSTOM':
        ...
        toList +=  '<td ' + tdcustom + '>' + makeButton('❌',`!dmr --TO-RemoveCustom ${i}`,20) + '</td>';
      ...
      case 'CHAR':
        ...
        if(toTkn.get('layer') == 'objects'){
          ...
          toList += '<td ' + tdbase + '>' + makeButton(tknImg, `!dmr --PingToken-GM ${toObj[i].id}`, 40) + '<b>' + toTkn.get('name') + '</b></td>';                
        } else {
          // toList += '<td ' + tdbase + '>' + makeButton(tknImg, '!ping-token --' + toTkn.get('_id'), 40) + '<i>' + toTkn.get('name') + '</i></td>';
          toList += '<td ' + tdbase + '>' + makeButton(tknImg, `!dmr --PingToken-GM ${toObj[i].id}`, 40) + '<i>' + toTkn.get('name') + '</i></td>';                
        }

        // Col 3 (Commands)
        toList += '<td ' + tdbase + '>';
        toList += makeButton('❌',`!dmr --TO-Remove ${toObj[i].id}`);

        if(toTkn.get('layer') == 'objects'){
          toList += makeButton('🙈', '!token-mod --set layer|gmlayer --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh); 
        } else {
          toList += makeButton('🙉', '!token-mod --set layer|objects --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh);
        }

        toList += makeButton('📑', 'https://journal.roll20.net/character/' + toTkn.get('represents'));
        if (isnpc) {
          toList += makeButton('🖊', '!dmr --AddTokenGMNote ' + toTkn.get('_id') + ' "?{Token GM Note?|}"');  
        } else {
          toList += makeButton('🖊', '!dmr --AddCharGMNote ' + toTkn.get('represents') + ' "?{Character GM Note?|}"');
        }
  ...
  tokens.forEach(t => {
    ...
    if (tType == 'NPC' || tType == 'CHAR'){
      ...
      if(ndx<0){
        ...
        let btn_ping = makeButton(tImg, `!dmr --PingToken-GM ${t.id}`, 40);
        if (tType == 'NPC') {
          btnNPCs += btn_ping + t.get('name') + makeButton(' [+] ', `!group-init --ids ${t.get('_id')} &#13;!dmr --TOReport`,20);
          btnAddAllNPCs += t.get('_id') + ' ';
        } else {
          btnPCs += btn_ping + t.get('name') + makeButton(' [+] ', `!group-init --ids ${t.get('_id')} &#13;!dmr --TOReport`,20);
  ...
  btnAddAllNPCs = '!group-init --ids ' + btnAddAllNPCs + ' &#13;!dmr --TOReport';
  btnAddAllNPCs = makeButton(' [+] ', btnAddAllNPCs ,20) + '  ';
  btnAddAllPCs = '!group-init --ids ' + btnAddAllPCs + ' &#13;!dmr --TOReport';
  btnAddAllPCs = makeButton(' [+] ', btnAddAllPCs ,20) + '  ';

  toList += btns + '<hr><b><u>NPCs</u></b>' + btnAddAllNPCs + '<br>' + btnNPCs + '<br><b><u>Player Characters</u></b>' + btnAddAllPCs + '<br>' + btnPCs;
