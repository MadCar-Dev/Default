...
//  2. Add saving throw and ability check throws to the character sheet
...
=======
          if(toTkn.get('show_tooltip')){
            toList += '<td ' + tdbase + '>';
            toList += makeButton('🖊', '!token-mod --set tooltip|"?{Edit Tooltip|' + tt + '}" --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh) 
            toList += makeButton('❌', '!token-mod --set tooltip|?{Edit Tooltip| --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh) 
            toList += makeButton('🙈', '!token-mod --off show_tooltip --ignore-selected --ids ' + toTkn.get('_id') + cmdRefresh);
            toList += '<b>' + tt + '</b></td>'; 
          } else {
...
=======
          let btn_ping = makeButton(tImg, `!tor --PingToken-GM ${t.id}`, 40);
          if (tType == 'NPC') {
            btnNPCs += btn_ping + t.get('name') + makeButton(' [+] ', `!group-init --ids ${t.get('_id')} &#13;!tor --TOReport`,20);
            // btnNPCs += btn_ping + t.get('name') + makeButton(' [+] ', `!&#13;&#37;{${t.get('name')}|Initiative} ${cmdRefresh}`);
            btnAddAllNPCs += t.get('_id') + ' ';
          } else {
            btnPCs += btn_ping + t.get('name') + makeButton(' [+] ', `!group-init --ids ${t.get('_id')} &#13;!tor --TOReport`,20);
            // btnPCs += btn_ping + t.get('name') + makeButton(' [+] ', `!&#13;&#37;{${t.get('name')}|Initiative} ${cmdRefresh}`);
            btnAddAllPCs += t.get('_id') + ' ';
>>>>>>> Stashed changes
          }
        }
